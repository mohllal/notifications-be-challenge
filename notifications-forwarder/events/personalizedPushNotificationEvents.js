...
queueEvents.on('waiting', async ({ jobId }) => {
  ...
  logger.info(`[${personalizedPushNotificationQueueName}] job with ID ${jobId} is waiting`);
});

queueEvents.on('active', async ({ jobId, prev }) => {
  logger.info(`[${personalizedPushNotificationQueueName}] job with ID ${jobId} is now active; previous status was ${prev}`);
});
...
queueEvents.on('completed', async ({ jobId, returnvalue }) => {
  ...
  logger.info(`[${personalizedPushNotificationQueueName}] job with ID ${jobId} has completed and returned ${returnvalue}`);
});
...
queueEvents.on('failed', async ({ jobId, failedReason }) => {
  ...
  logger.info(`[${personalizedPushNotificationQueueName}] job with ID ${jobId} has failed with reason ${failedReason}`);
});
